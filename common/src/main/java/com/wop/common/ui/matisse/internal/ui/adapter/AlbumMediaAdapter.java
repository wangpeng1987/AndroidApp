...
import com.wop.common.ui.matisse.internal.ui.widget.MediaGrid;
...
public class AlbumMediaAdapter extends
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_CAPTURE) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_album_capture_, parent, false);
            ...
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.include_album_grid, parent, false);
    ...
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Cursor cursor) {
        if (holder instanceof CaptureViewHolder) {
            ...
            if (!SelectionSpec.getInstance().isMultiSelect) {
                mediaViewHolder.mMediaGrid.hiddenCheckView();
            }
        }
    }
    ...
    private int getImageResize(Context context) {
        if (mImageResize == 0) {
            ...
            int availableWidth = screenWidth - context.getResources().getDimensionPixelSize(
                    R.dimen.dimen_2_dp) * (spanCount - 1);
